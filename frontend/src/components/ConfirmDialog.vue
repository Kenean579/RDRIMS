    ...
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <div class="flex justify-end gap-3 mt-4">
          <button @click="$emit('cancel')" class="px-4 py-2 text-sm border border-gray-300 rounded-2xl hover:bg-gray-50">Cancel</button>
          <button @click="$emit('confirm')" :class="{ 'bg-red-600 hover:bg-red-700': variant === 'danger', 'bg-blue-600 hover:bg-blue-700': variant === 'primary', 'bg-amber-600 hover:bg-amber-700': variant === 'warning' }"
            class="px-4 py-2 text-sm text-white rounded-2xl">{{ confirmText }}</button>
        </div>
      </div>
    </div>
  </Teleport>

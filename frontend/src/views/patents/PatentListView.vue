  ...
  <div class="flex flex-col gap-8 pb-12 animate-fade card">
      ...
      <div v-for="patent in patents" :key="patent.id" class="card p-6 flex flex-col group card-hover relative border-l-4 border-l-transparent hover:border-l-brand transition-all">
        <div class="flex items-start justify-between mb-5">
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[10px] font-black text-slate-400 capitalize tracking-widest">Inventors: {{ patent.inventors }}</p>
          </div>
        ...
        <div class="mt-auto pt-5 border-t border-slate-50 flex items-center justify-between">
          <span class="text-[10px] font-black text-brand capitalize tracking-widest bg-brand-light px-2.5 py-1 rounded-lg">{{ patent.licenses?.length || 0 }} Licenses</span>
          <div class="flex gap-2">
            <button @click="editPatent(patent)" class="btn btn-ghost hover:bg-slate-100 text-[11px] font-black capitalize tracking-widest py-2">Edit</button>
            <button @click="confirmDelete(patent)" class="btn btn-ghost text-red-500 hover:bg-red-50 text-[11px] font-black capitalize tracking-widest py-2">Delete</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <Modal :show="showCreate || !!editingPatent" :title="editingPatent ? 'Edit Patent' : 'Add New Patent'" @close="closeModal">
      <form @submit.prevent="savePatent" class="space-y-6">
        <div>
          <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Patent Title *</label>
          <input v-model="form.title" type="text" required class="input h-12 font-bold" placeholder="e.g. New Solar Panel Tech" />
        </div>
        <div>
          <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Inventors *</label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Filing Date *</label>
            <input v-model="form.filing_date" type="date" required class="input h-12 font-bold" />
          </div>
          <div>
            <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Patent Number</label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Project</label>

  ...
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-5">
      ...
      <div class="lg:col-span-1 space-y-2">
        <label class="block text-[10px] font-medium text-slate-400 mb-3 ml-1">Select Table</label>
        <button 
          ...
          <div class="flex items-center justify-between bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
             <div class="flex items-center gap-3">
               ...
               <div>
                 ...
                 <p class="text-[9px] font-medium text-slate-400">Table: {{ activeTable.id }}</p>
               </div>
             </div>
             <button @click="showAdd = true" class="btn btn-primary h-10 px-6 text-[10px] font-medium">Add Option</button>
          </div>
          ...
          <div v-else class="card overflow-hidden">
            <div class="overflow-x-auto">
              <table class="table-auto">
                <thead>
                  <tr>
                    <th class="w-20 pl-8 py-4 text-[10px] font-medium text-slate-400">ID</th>
                    <th class="py-4 text-[10px] font-medium text-slate-400">Name / Label</th>
                  ...
                  <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50/30 transition-colors group">
                    ...
                    <td class="pr-8 py-4 text-right">
                      <div class="flex justify-end gap-2">
                        <button @click="editItem(item)" class="btn btn-ghost text-[10px] font-medium py-1.5 px-3">Rename</button>
                        <button @click="confirmDelete(item)" class="btn btn-ghost text-rose-500 hover:bg-rose-50 text-[10px] font-medium py-1.5 px-3">Delete</button>
                      </div>
                    </td>
                  </tr>
                </tbody>
    ...
    <Modal :show="showAdd || !!editingItem" :title="editingItem ? 'Edit Lookup' : 'Add New Option'" @close="closeModal">
      <form @submit.prevent="saveItem" class="space-y-4">
        <div>
          <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Option Name *</label>

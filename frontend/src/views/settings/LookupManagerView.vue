  ...
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-5">
      ...
      <div class="lg:col-span-1 space-y-2">
        <label class="block text-[10px] font-bold text-slate-400 capitalize tracking-widest mb-3 ml-1">Select Table</label>
        <button 
          v-for="table in tables" :key="table.id"
          @click="selectTable(table)"
          class="w-full text-left px-4 py-3 rounded-xl text-xs font-bold transition-all relative group overflow-hidden"
          :class="activeTable?.id === table.id ? 'bg-brand text-white shadow-lg shadow-brand/20' : 'bg-slate-50 text-slate-600 hover:bg-white hover:shadow-md border border-transparent hover:border-slate-100'"
        >
          {{ table.label }}
      ...
      <div class="lg:col-span-3 space-y-6">
        <div v-if="!activeTable" class="card p-5 text-center">
        ...
        <div v-else class="space-y-6 animate-fade-in">
          <div class="flex items-center justify-between bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
             <div class="flex items-center gap-3">
               <div class="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-brand">
               ...
               <div>
                 ...
                 <p class="text-[9px] font-bold text-slate-400 capitalize tracking-widest">Table: {{ activeTable.id }}</p>
               </div>
             </div>
             <button @click="showAdd = true" class="btn btn-primary h-10 px-6 text-[10px] font-bold capitalize tracking-widest shadow-lg shadow-blue-500/10">Add Option</button>
          </div>
          ...
          <div v-else class="card overflow-hidden">
            <div class="overflow-x-auto">
              <table class="table-auto">
                <thead>
                  <tr>
                    <th class="w-20 pl-8 py-4 text-[10px] font-bold capitalize tracking-widest text-slate-400">ID</th>
                    <th class="py-4 text-[10px] font-bold capitalize tracking-widest text-slate-400">Name / Label</th>
                ...
                <tbody class="divide-y divide-slate-50">
                  <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50/30 transition-colors group">
                    ...
                    <td class="py-4">
                      <span class="px-3 py-1 bg-slate-100 text-slate-700 text-xs font-bold capitalize tracking-tight rounded-lg border border-slate-200">{{ item.name }}</span>
                    </td>
                    <td class="pr-8 py-4 text-right">
                      <div class="flex justify-end gap-2">
                        <button @click="editItem(item)" class="btn btn-ghost text-[10px] font-bold capitalize tracking-widest py-1.5 px-3">Rename</button>
                        <button @click="confirmDelete(item)" class="btn btn-ghost text-rose-500 hover:bg-rose-50 text-[10px] font-bold capitalize tracking-widest py-1.5 px-3">Delete</button>
                      </div>
                    </td>
                  </tr>
                </tbody>
            ...
            <div v-if="items.length === 0" class="p-6 text-center text-xs font-bold text-slate-400 capitalize tracking-widest italic">No records in this lookup.</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <Modal :show="showAdd || !!editingItem" :title="editingItem ? 'Edit Lookup' : 'Add New Option'" @close="closeModal">
      <form @submit.prevent="saveItem" class="space-y-4">
        <div>
          <label class="block text-[11px] text-slate-500 font-bold capitalize tracking-widest mb-2 ml-1">Option Name *</label>
        ...
        <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
           ...
           <button type="submit" class="btn btn-primary px-5 shadow-lg shadow-blue-500/20">Save</button>
        </div>
      </form>
    </Modal>

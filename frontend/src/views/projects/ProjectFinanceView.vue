<template>
  <div class="flex flex-col gap-5 pb-6 animate-fade">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div>
        <router-link :to="`/app/projects/${route.params.id}`" class="flex items-center gap-2 text-brand font-bold capitalize tracking-widest text-[10px] mb-3 hover:translate-x-1 transition-transform">
      ...
      <div>
        <button v-if="auth.hasRole('super_admin', 'resource_admin', 'finance_officer') || isPI" @click="showAddExpense = true" class="btn bg-brand hover:bg-brand-dark text-white text-[11px] font-bold capitalize tracking-widest h-11 px-6 shadow-lg shadow-brand/30 flex items-center gap-2">
    ...
    <div v-else class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
          <p class="text-[10px] capitalize tracking-widest text-slate-400 mb-2 font-bold">Total Budget</p>
        ...
        <div class="bg-emerald-50 rounded-2xl p-6 border border-emerald-200 shadow-sm">
          <p class="text-[10px] capitalize tracking-widest text-emerald-600 mb-2 font-bold">Remaining Balance</p>
        ...
        <div class="bg-rose-50 rounded-2xl p-6 border border-rose-200 shadow-sm">
          <p class="text-[10px] capitalize tracking-widest text-rose-600 mb-2 font-bold">Total Spent</p>
        ...
        <div class="bg-indigo-50 rounded-2xl p-6 border border-indigo-200 shadow-sm">
          <p class="text-[10px] capitalize tracking-widest text-indigo-600 mb-2 font-bold">Total Disbursed</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
          <h2 class="text-sm font-bold capitalize tracking-widest text-slate-700">Expense Log</h2>
        </div>
        ...
        <table v-else class="w-full text-left">
          <thead class="bg-slate-50 text-[10px] font-bold capitalize tracking-widest text-slate-500">
            <tr>
          ...
          <tbody class="text-sm">
            <tr v-for="exp in project.expenses" :key="exp.id" class="border-t border-slate-100 hover:bg-slate-50/50 transition-colors">
              ...
              <td class="px-6 py-4"><span class="px-2 py-1 bg-slate-100 rounded text-[10px] font-bold text-slate-500 capitalize tracking-widest">{{ exp.category?.name || 'General' }}</span></td>
    ...
    <Modal :show="showAddExpense" title="Record New Expense" @close="showAddExpense = false">
      <form @submit.prevent="addExpense" class="space-y-6">
        <div>
          <label class="block text-[10px] font-bold text-slate-500 capitalize tracking-widest mb-2 ml-1">Expense Title / Description *</label>
        ...
        <div class="grid grid-cols-2 gap-6">
          <div>
            <label class="block text-[10px] font-bold text-slate-500 capitalize tracking-widest mb-2 ml-1">Amount (ETB) *</label>
          ...
          <div>
            <label class="block text-[10px] font-bold text-slate-500 capitalize tracking-widest mb-2 ml-1">Date *</label>
        ...
        <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
          <button type="button" @click="showAddExpense = false" class="btn btn-secondary px-5 h-11 text-[11px] font-bold capitalize tracking-widest">Cancel</button>
          <button type="submit" :disabled="submitting" class="btn btn-primary px-5 h-11 shadow-lg shadow-brand/20 text-[11px] font-bold capitalize tracking-widest">

<template>
  <div class="flex flex-col gap-8 pb-12 animate-fade">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div>
        <router-link :to="`/app/projects/${route.params.id}`" class="flex items-center gap-2 text-brand font-black uppercase tracking-widest text-[10px] mb-3 hover:translate-x-1 transition-transform">
        ...
        <p class="text-slate-500 font-medium mt-1 uppercase tracking-widest text-[9px]">Expense Tracking & Disbursals</p>
      </div>
      <div>
        <button v-if="auth.hasRole('super_admin', 'resource_admin', 'finance_officer') || isPI" @click="showAddExpense = true" class="btn bg-brand hover:bg-brand-dark text-white text-[11px] font-black uppercase tracking-widest h-11 px-6 shadow-lg shadow-brand/30 flex items-center gap-2">
    ...
    <div v-else class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
          <p class="text-[10px] uppercase tracking-widest text-slate-400 mb-2 font-black">Total Budget</p>
        ...
        <div class="bg-emerald-50 rounded-2xl p-6 border border-emerald-200 shadow-sm">
          <p class="text-[10px] uppercase tracking-widest text-emerald-600 mb-2 font-black">Remaining Balance</p>
        ...
        <div class="bg-rose-50 rounded-2xl p-6 border border-rose-200 shadow-sm">
          <p class="text-[10px] uppercase tracking-widest text-rose-600 mb-2 font-black">Total Spent</p>
        ...
        <div class="bg-indigo-50 rounded-2xl p-6 border border-indigo-200 shadow-sm">
          <p class="text-[10px] uppercase tracking-widest text-indigo-600 mb-2 font-black">Total Disbursed</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
          <h2 class="text-sm font-black uppercase tracking-widest text-slate-700">Expense Log</h2>
        </div>
        ...
        <table v-else class="w-full text-left">
          ...
          <tbody class="text-sm">
            <tr v-for="exp in project.expenses" :key="exp.id" class="border-t border-slate-100 hover:bg-slate-50/50 transition-colors">
              ...
              <td class="px-6 py-4"><span class="px-2 py-1 bg-slate-100 rounded text-[10px] font-black text-slate-500 uppercase tracking-widest">{{ exp.category?.name || 'General' }}</span></td>
    ...
    <Modal :show="showAddExpense" title="Record New Expense" @close="showAddExpense = false">
      <form @submit.prevent="addExpense" class="space-y-6">
        <div>
          <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Expense Title / Description *</label>
        ...
        <div class="grid grid-cols-2 gap-6">
          <div>
            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Amount (ETB) *</label>
          ...
          <div>
            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Date *</label>
        ...
        <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
          <button type="button" @click="showAddExpense = false" class="btn btn-secondary px-8 h-11 text-[11px] font-black uppercase tracking-widest">Cancel</button>
          <button type="submit" :disabled="submitting" class="btn btn-primary px-10 h-11 shadow-lg shadow-brand/20 text-[11px] font-black uppercase tracking-widest">

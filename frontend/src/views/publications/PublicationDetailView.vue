<template>
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div>
        <router-link to="/app/publications" class="flex items-center gap-2 text-brand font-bold text-[10px] mb-3 hover:translate-x-1 transition-transform">
        ...
        <div class="flex items-center gap-3 mt-3">
          <span class="px-2.5 py-1 bg-indigo-50 text-indigo-600 border border-indigo-100 text-[10px] font-medium rounded-2xl">{{ pub.type?.name || 'Journal Article' }}</span>
          <span v-if="pub.doi" class="text-[10px] text-slate-400 font-medium">DOI: {{ pub.doi }}</span>
        </div>
      </div>
    </div>
          ...
          <div class="card p-8-500/20">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
              <div class="sm:col-span-2">
                <dt class="text-[10px] font-medium text-slate-400 mb-1.5 ml-1">Full Title</dt>
              ...
              <div>
                <dt class="text-[10px] font-medium text-slate-400 mb-1.5 ml-1">Source Venue</dt>
                <dd class="p-4 rounded-2xl bg-white border border-slate-100 text-slate-700">
                   ...
                   <p class="text-[10px] opacity-60">Published: {{ pub.publication_year }}</p>
                </dd>
              </div>
              <div>
                <dt class="text-[10px] font-medium text-slate-400 mb-1.5 ml-1">Impact & Mentions</dt>
        ...
        <div class="space-y-8">
          <div class="card p-8 bg-slate-50/50">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-[10px] font-medium text-slate-400">Authorship Team</h2>
              <button @click="showAddAuthor = true" class="text-[10px] font-medium text-brand hover:underline">+ Add Member</button>
            </div>
            ...
            <div v-if="pub.authors?.length" class="space-y-4">
              <div v-for="author in pub.authors" :key="author.id" class="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 group shadow-sm">
                <div class="min-w-0">
                  ...
                  <p class="text-[9px] text-slate-400">Priority Order: {{ author.author_order }}</p>
                </div>
            ...
            <div v-else class="p-5 text-center bg-white rounded-2xl border border-dashed border-slate-100">
               <p class="text-[10px] font-medium text-slate-400">No authors added.</p>
            </div>
          </div>
          ...
          <div v-if="pub.url" class="card p-8 bg-slate-100 text-white shadow-xl shadow-indigo-500/20">
             ...
             <a :href="pub.url" target="_blank" class="btn w-full bg-white text-indigo-700 text-[11px] font-medium h-11 border-0 shadow-lg">Open Full Text</a>
          </div>
        </div>
      </div>
    </template>

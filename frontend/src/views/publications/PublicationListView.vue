  ...
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <div class="card p-8 bg-slate-50/50">
      <div class="flex flex-col sm:flex-row gap-5 items-start">
        ...
        <div class="w-full sm:w-56">
          <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Category</label>
      ...
      <div v-for="pub in publications" :key="pub.id" class="bg-white rounded-2xl border border-slate-100 p-4 flex flex-col group hover:shadow-md transition-all cursor-pointer">
        <div class="flex items-start justify-between mb-4">
          <div class="flex-1 pr-4 min-w-0">
            <div class="flex items-center gap-3 mb-2.5">
              <span class="px-2.5 py-0.5 bg-indigo-50 text-indigo-600 border border-indigo-100 text-[10px] font-medium rounded-md">{{ pub.type?.name || 'Journal Article' }}</span>
              <span v-if="pub.doi" class="text-[9px] text-slate-400 font-medium tracking-widest  truncate max-w-[150px]">DOI: {{ pub.doi }}</span>
            </div>
        ...
        <div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
           <div class="flex flex-col gap-1">
             <span class="flex items-center gap-1.5 text-[9px] font-medium text-brand">
                <i class="fas fa-university opacity-70"></i>
                {{ pub.journal_name || pub.conference_name || 'RDRIMS Repository' }}
             </span>
             <span class="flex items-center gap-1.5 text-[9px] font-medium text-slate-400">
           ...
           <div class="flex gap-2">
             <a v-if="pub.url" :href="pub.url" target="_blank" class="btn btn-ghost text-[10px] font-medium py-1.5 px-3 text-brand hover:bg-brand-light">
               Read Link
             </a>
    ...
    <Modal :show="showAdd" title="Add New Publication" size="lg" @close="showAdd = false">
      <form @submit.prevent="savePublication" class="space-y-6">
        <div>
          <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Publication Title *</label>
          <input v-model="form.title" type="text" required class="input h-12 font-bold" placeholder="e.g. New Research in Science" />
        </div>
        
        <div>
          <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Authors *</label>
          <input v-model="form.authors" type="text" required placeholder="Names (e.g. Abebe K., Smith J.)" class="input h-12 font-bold" />
          <p class="text-[10px] text-slate-400 font-medium mt-2 ml-1">Separate names with commas</p>
        </div>
        
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Year published *</label>
        ...
        <div>
          <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Journal or Conference Name</label>
        ...
        <div class="flex justify-end gap-3 pt-6 mt-6 border-t border-slate-100">
          <button type="button" @click="showAdd = false" class="btn btn-secondary px-6 font-bold tracking-widest  text-[11px]">Cancel</button>
          <button type="submit" class="btn btn-primary px-5 font-bold tracking-widest  text-[11px]">Save Publication</button>
        </div>
      </form>
    </Modal>
  </div>

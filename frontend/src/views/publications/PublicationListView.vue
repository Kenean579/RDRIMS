  ...
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <div class="card p-5 bg-slate-50/50">
      <div class="flex flex-col sm:flex-row gap-5 items-start">
        ...
        <div class="w-full sm:w-56">
          <label class="block text-[11px] text-slate-500 font-bold capitalize tracking-widest mb-2 ml-1">Category</label>
      ...
      <div v-for="pub in publications" :key="pub.id" class="card p-6 flex flex-col group card-hover relative overflow-hidden border-l-4 border-l-brand hover:border-l-indigo-600 transition-all">
        <div class="flex items-start justify-between mb-4">
          <div class="flex-1 pr-4 min-w-0">
            <div class="flex items-center gap-3 mb-2.5">
              <span class="px-2.5 py-0.5 bg-indigo-50 text-indigo-600 border border-indigo-100 text-[10px] font-bold capitalize tracking-widest rounded-md">{{ pub.type?.name || 'Journal Article' }}</span>
              <span v-if="pub.doi" class="text-[9px] text-slate-400 font-bold tracking-widest capitalize truncate max-w-[150px]">DOI: {{ pub.doi }}</span>
            </div>
        ...
        <div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
           <div class="flex flex-col gap-1">
             <span class="flex items-center gap-1.5 text-[9px] font-bold capitalize tracking-widest text-brand">
                <i class="fas fa-university opacity-70"></i>
                {{ pub.journal_name || pub.conference_name || 'RDRIMS Repository' }}
             </span>
             <span class="flex items-center gap-1.5 text-[9px] font-bold capitalize tracking-widest text-slate-400">
           ...
           <div class="flex gap-2">
             <a v-if="pub.url" :href="pub.url" target="_blank" class="btn btn-ghost text-[10px] font-bold capitalize tracking-widest py-1.5 px-3 text-brand hover:bg-brand-light">
               Read Link
             </a>
    ...
    <Modal :show="showAdd" title="Add New Publication" size="lg" @close="showAdd = false">
      <form @submit.prevent="savePublication" class="space-y-6">
        <div>
          <label class="block text-[11px] text-slate-500 font-bold capitalize tracking-widest mb-2 ml-1">Publication Title *</label>
          <input v-model="form.title" type="text" required class="input h-12 font-bold" placeholder="e.g. New Research in Science" />
        </div>
        
        <div>
          <label class="block text-[11px] text-slate-500 font-bold capitalize tracking-widest mb-2 ml-1">Authors *</label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            <label class="block text-[11px] text-slate-500 font-bold capitalize tracking-widest mb-2 ml-1">Year published *</label>
        ...
        <div>
          <label class="block text-[11px] text-slate-500 font-bold capitalize tracking-widest mb-2 ml-1">Journal or Conference Name</label>
        ...
        <div class="flex justify-end gap-3 pt-6 mt-6 border-t border-slate-100">
          <button type="button" @click="showAdd = false" class="btn btn-secondary px-6 font-bold tracking-widest capitalize text-[11px]">Cancel</button>
          <button type="submit" class="btn btn-primary px-5 shadow-lg shadow-blue-500/20 font-bold tracking-widest capitalize text-[11px]">Save Publication</button>
        </div>
      </form>
    </Modal>
  </div>

<template>
  <div class="flex flex-col gap-8 pb-12 animate-fade card">
    ...
    <div v-else-if="error" class="card border-rose-100 bg-rose-50/30 p-12 text-center shadow-xl shadow-rose-500/5">
       ...
       <h3 class="text-lg font-black text-slate-800 mb-2 capitalize tracking-tight">System Error</h3>
       <p class="text-sm text-rose-600 font-bold mb-6 capitalize tracking-widest text-[11px] leading-relaxed">{{ error }}</p>
       <button @click="fetchProblems" class="btn bg-rose-600 hover:bg-rose-700 text-white px-8 h-11 text-[11px] font-black capitalize tracking-widest border-0">Retry</button>
    </div>
      ...
      <div v-for="p in problems" :key="p.id" class="card group card-hover flex flex-col border-l-4 border-l-brand/20 hover:border-l-brand transition-all overflow-hidden font-bold">
        <div class="p-6 flex-1">
          ...
          <div class="flex flex-col gap-3">
            <div class="flex items-center gap-2.5 text-[10px] font-black text-slate-400 capitalize tracking-widest bg-slate-50 p-2.5 rounded-xl border border-slate-100 shadow-inner">
              <svg class="w-4 h-4 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              {{ p.location }}
            </div>
            <div v-if="p.claimed_by" class="flex items-center gap-2.5 text-[10px] font-black text-slate-400 capitalize tracking-widest bg-slate-50 p-2.5 rounded-xl border border-slate-100 shadow-inner">
            ...
            <div v-if="p.rating" class="flex items-center gap-2 text-[10px] font-black text-slate-400 capitalize tracking-widest bg-amber-50/50 p-2.5 rounded-xl border border-amber-100 shadow-inner">
        ...
        <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex gap-2">
          <button v-if="p.status?.name === 'open'" @click="claimProblem(p)" class="flex-1 btn btn-primary justify-center text-[11px] font-black capitalize tracking-widest h-10 shadow-lg shadow-blue-500/20">Help Out</button>
          <button v-if="p.status?.name === 'claimed' && p.claimed_by?.id === auth.user?.id" @click="completeProblem(p)" class="flex-1 btn btn-primary justify-center text-[11px] font-black capitalize tracking-widest h-10 shadow-lg shadow-blue-500/20">Finish</button>
          <button v-if="p.status?.name === 'completed' && !p.feedback" @click="openFeedback(p)" class="flex-1 btn btn-secondary justify-center text-[11px] font-black capitalize tracking-widest h-10 px-4">Give Feedback</button>
          <span v-if="p.status?.name === 'completed' && p.feedback" class="flex-1 text-center py-2 text-[10px] font-black capitalize tracking-widest text-emerald-600 bg-emerald-50 rounded-xl border border-emerald-100">Task Completed ✓</span>
        </div>
      </div>
    </div>
    ...
    <Modal :show="showCreate" title="Report an Issue" size="md" @close="showCreate = false">
      <form @submit.prevent="submitProblem" class="space-y-6">
        <div>
          <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">What is the problem? *</label>
          <input v-model="form.title" type="text" required class="input h-12 font-bold" placeholder="Give it a clear title..." />
        </div>
        <div>
          <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">More Details *</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Where is it? *</label>
            <input v-model="form.location" type="text" required class="input h-12 font-bold" placeholder="e.g. Region/Zone" />
          </div>
          <div>
            <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Phone / Email</label>
            <input v-model="form.contact_info" type="text" class="input h-12 font-bold" placeholder="How to reach you" />
          </div>
        </div>
        <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
          <button type="button" @click="showCreate = false" class="btn btn-secondary px-8 h-12 text-[11px] font-black capitalize tracking-widest">Cancel</button>
          <button type="submit" class="btn btn-primary px-12 h-12 text-[11px] font-black capitalize tracking-widest shadow-xl shadow-blue-500/20">Submit Final Report</button>
        </div>
      </form>
    </Modal>
    ...
    <Modal :show="showFeedback" title="Leave a Review" @close="showFeedback = false">
      <form @submit.prevent="submitFeedback" class="space-y-6">
        <div>
          <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-3 ml-1">How would you rate the help?</label>
        ...
        <div>
          <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Share your experience *</label>
        ...
        <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
          <button type="button" @click="showFeedback = false" class="btn btn-secondary px-8 h-12 text-[11px] font-black capitalize tracking-widest">Cancel</button>
          <button type="submit" class="btn btn-primary px-12 h-12 text-[11px] font-black capitalize tracking-widest shadow-xl shadow-blue-500/20">Send Review</button>
        </div>
      </form>
    </Modal>  </div>
</template>

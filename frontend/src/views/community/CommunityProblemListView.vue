<template>
  <div class="flex flex-col gap-8 animate-fade pb-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 px-1">
      ...
      <button @click="showCreate = true" class="btn btn-primary h-11 px-6 text-[11px] font-bold gap-2 shrink-0">
    ...
    <div class="flex items-center gap-3 px-1 flex-wrap">
      <button
        ...
        class="px-4 py-2 rounded-2xl text-[11px] font-bold border transition-all"
    ...
    <div v-else-if="error" class="card border-rose-100 bg-rose-50/30 p-10 text-center">
      ...
      <button @click="fetchProblems" class="btn btn-primary px-6 h-11 text-[11px] font-bold">Retry</button>
    </div>
    ...
    <div v-else-if="problems.length === 0" class="card p-16 text-center">
      ...
      <button @click="showCreate = true" class="btn btn-primary px-8 h-11 text-[11px] font-bold shadow-lg shadow-brand/20">Report First Issue</button>
    </div>
      ...
      >
        ...
        <div class="p-6 flex-1">
          <div class="flex items-start justify-between gap-4 mb-4">
            ...
            <span
              class="px-3 py-1 rounded-full text-[10px] font-bold shrink-0"
              :class="{
          ...
          <div class="flex flex-col gap-2.5">
            <div class="flex items-center gap-2.5 text-[11px] font-medium text-slate-500 bg-slate-50 px-3 py-2.5 rounded-2xl border border-slate-100">
              <svg class="w-4 h-4 text-brand shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              {{ p.location }}
            </div>
            <div v-if="p.claimed_by" class="flex items-center gap-2.5 text-[11px] font-medium text-slate-500 bg-emerald-50 px-3 py-2.5 rounded-2xl border border-emerald-100">
            ...
            <div v-if="p.rating" class="flex items-center gap-2.5 text-[11px] font-medium text-slate-500 bg-amber-50 px-3 py-2.5 rounded-2xl border border-amber-100">
              <span class="text-amber-500 text-sm">★</span>
              Rating: <span class="font-bold text-amber-700">{{ p.rating }} / 5</span>
              <span v-if="p.results_summary" class="ml-auto italic text-[10px] text-slate-400 line-clamp-1">{{ p.results_summary }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="px-6 py-4 bg-slate-50/60 border-t border-slate-100 flex gap-2 flex-wrap">
          ...
          <button
            ...
            class="flex-1 btn btn-primary justify-center text-[11px] font-bold h-10 min-w-[100px]"
          >Help Out</button>
          ...
          <button
            v-if="p.status?.name === 'completed' && !p.results_summary && p.submitted_by?.id === auth.user?.id"
            @click="openFeedback(p)"
            class="flex-1 btn btn-secondary justify-center text-[11px] font-bold h-10 min-w-[100px]"
          >Leave Feedback</button>

          <!-- Completed with feedback -->
          <span
            v-if="p.status?.name === 'completed' && p.results_summary"
            class="flex-1 text-center py-2 text-[10px] font-bold text-emerald-700 bg-emerald-50 rounded-2xl border border-emerald-100 flex items-center justify-center gap-2"
          >
    ...
    <Modal :show="showCreate" title="Report a Community Issue" size="md" @close="closeCreate">
      <form @submit.prevent="submitProblem" class="space-y-5">
        <div>
          <label class="block text-[11px] text-slate-500 font-bold mb-2 ml-1">Problem Title *</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-[11px] text-slate-500 font-bold mb-2 ml-1">Location *</label>
            <input v-model="form.location" type="text" required class="input h-12 font-bold" placeholder="e.g. Zone 4, Addis Ababa" />
          </div>
          <div>
            <label class="block text-[11px] text-slate-500 font-bold mb-2 ml-1">Contact Info</label>
        ...
        <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
          ...
          <label for="anonymous" class="text-[11px] font-bold text-slate-600 cursor-pointer">Submit anonymously (your name will be hidden)</label>
        </div>
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
          <button type="button" @click="closeCreate" class="btn btn-secondary px-6 h-11 text-[11px] font-bold">Cancel</button>
          <button type="submit" class="btn btn-primary px-8 h-11 text-[11px] font-bold shadow-lg shadow-brand/20">Submit Report</button>
        </div>
      </form>
    </Modal>
    ...
    <Modal :show="showFeedback" title="Leave Feedback" size="sm" @close="closeFeedback">
      <form @submit.prevent="submitFeedback" class="space-y-5">
        <div>
          <label class="block text-[11px] text-slate-500 font-bold mb-3 ml-1">How would you rate the help?</label>
        ...
        <div>
          <label class="block text-[11px] text-slate-500 font-bold mb-2 ml-1">Results Summary *</label>
          <textarea v-model="feedbackForm.results_summary" required rows="3" class="input resize-none pt-4 font-medium" placeholder="How was the issue resolved? Any comments..."></textarea>
        </div>
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
          <button type="button" @click="closeFeedback" class="btn btn-secondary px-6 h-11 text-[11px] font-bold">Cancel</button>
          <button type="submit" class="btn btn-primary px-8 h-11 text-[11px] font-bold shadow-lg shadow-brand/20">Submit Feedback</button>
        </div>
      </form>
    </Modal>
...
const feedbackForm = reactive({
  rating: 3,
  results_summary: '',   // matches backend field name
})
...
async function submitFeedback() {
  try {
    // Backend expects: results_summary + rating
    await api.post(`/community-problems/${feedbackProblem.value.id}/feedback`, {
      results_summary: feedbackForm.results_summary,

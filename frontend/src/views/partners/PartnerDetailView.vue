<template>
  <div card>
    <div class="mb-6">
      <router-link to="/partners" class="text-sm text-blue-600 hover:underline mb-2 inline-block">← Back to Partners</router-link>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div>
          <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
            <div class="w-16 h-16 bg-blue-100 rounded-lg flex items-center justify-center text-2xl mx-auto mb-4">🤝</div>
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
            ...
            <div v-if="partner.mo_us?.length" class="space-y-2">
              <div v-for="mou in partner.mo_us" :key="mou.id" class="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                <div>
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
            ...
            <div v-if="partner.outputs?.length" class="space-y-2">
              <div v-for="o in partner.outputs" :key="o.id" class="p-3 border border-gray-200 rounded-lg">
    ...
    <Modal :show="showMoUForm" title="Add MoU" @close="showMoUForm = false">
      <form @submit.prevent="saveMoU" class="space-y-4">
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm font-medium text-gray-700 mb-1">Start Date *</label><input v-model="mouForm.start_date" type="date" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none" /></div>
          <div><label class="block text-sm font-medium text-gray-700 mb-1">End Date *</label><input v-model="mouForm.end_date" type="date" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none" /></div>
        </div>
        <div class="flex justify-end gap-3"><button type="button" @click="showMoUForm = false" class="px-4 py-2 text-sm border border-gray-300 rounded-lg">Cancel</button><button type="submit" class="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg">Save</button></div>
      </form>
    </Modal>
  </div>
</template>

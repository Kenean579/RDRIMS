  ...
  <div class="flex flex-col gap-8 pb-12 animate-fade card">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
      <div class="card p-6 bg-linear-to-br from-brand to-indigo-600 border-0 text-white shadow-xl shadow-brand/20">
        <p class="text-[10px] font-black capitalize tracking-widest mb-1 opacity-80">Indexed Expertise</p>
        <p class="text-3xl font-black">{{ expertise.length }} Tags</p>
      </div>
    ...
    <div v-else class="card p-8 bg-white border-dashed border-2 border-slate-100">
      <div class="flex flex-wrap gap-4">
        <div v-for="exp in filteredExpertise" :key="exp.id" 
          ...
          <span class="text-xs font-black text-slate-700 tracking-tight group-hover:text-indigo-600 transition-colors capitalize">{{ exp.name }}</span>
    ...
    <Modal :show="showCreate" title="Add New Tag" size="md" @close="showCreate = false">
      <form @submit.prevent="saveExpertise" class="space-y-6">
        <div>
          <label class="block text-[11px] text-slate-500 font-black capitalize tracking-widest mb-2 ml-1">Tag Name *</label>

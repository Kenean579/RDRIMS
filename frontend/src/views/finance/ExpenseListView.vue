  ...
  <div class="flex flex-col gap-6 card">
          ...
          <tbody>
            <tr v-for="ex in expenses" :key="ex.id" class="group">
              ...
              <td class="text-sm text-slate-500">
                {{ ex.description }}
                <div class="text-xs text-brand mt-1 capitalize">{{ ex.budget_category }}</div>
              </td>
    ...
    <Modal :show="showCreate" title="Record New Expense" @close="showCreate = false">
      <form @submit.prevent="saveExpense" class="space-y-5 px-1 py-1">
        <div>
          <label class="block text-xs text-slate-500 font-medium  tracking-wider mb-1.5 ml-1">Project *</label>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs text-slate-500 font-medium  tracking-wider mb-1.5 ml-1">Amount *</label>
            <input v-model.number="form.amount" type="number" step="0.01" required class="input" />
          </div>
          <div>
            <label class="block text-xs text-slate-500 font-medium  tracking-wider mb-1.5 ml-1">Budget Category *</label>
            <select v-model="form.budget_category" required class="input">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2">
            <label class="block text-xs text-slate-500 font-medium  tracking-wider mb-1.5 ml-1">Date *</label>
...
const form = reactive({ project_id: '', amount: '', expense_date: new Date().toISOString().split('T')[0], description: '', budget_category: 'other' })
...
async function saveExpense() {
  try {
    ...
    showCreate.value = false; Object.assign(form, { project_id: '', amount: '', description: '', budget_category: 'other' }); fetchExpenses()

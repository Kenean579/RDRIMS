<template>
  <div class="flex flex-col gap-6 pb-6 animate-fade card">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-xl font-bold text-slate-900 tracking-tight">Assigned Proposals</h1>
        <p class="text-slate-500 font-medium mt-1 text-[11px]">Proposals assigned to you for scientific and technical evaluation.</p>
      </div>
      <div class="flex items-center gap-2">
         <div v-if="proposals.length" class="px-3 py-1 bg-amber-50 text-amber-600 border border-amber-100 rounded-lg text-[9px] font-bold  tracking-widest">
    ...
    <div v-else-if="error" class="card p-8 border-rose-100 bg-rose-50/20 text-center">
       ...
       <button @click="fetchProposals(1)" class="btn btn-secondary px-6 h-10 text-[10px]">Retry Synchronize</button>
    </div>
        ...
        <div class="flex items-start justify-between mb-4">
          <div class="flex-1 pr-4 min-w-0">
            ...
            <div class="flex items-center gap-2 mt-2">
               <span class="px-2 py-0.5 bg-slate-100 text-slate-500 text-[9px] font-bold rounded  tracking-tighter border border-slate-100">ID: {{ String(p.id).padStart(4, '0') }}</span>
               <span class="px-2 py-0.5 bg-brand/10 text-brand text-[9px] font-bold rounded  tracking-tighter border border-brand/10">{{ p.thematic_area?.name || 'General' }}</span>
            </div>
          </div>
          <div class="shrink-0 flex flex-col items-end gap-2">
            ...
            <div v-if="p.reviewPivot?.overall_score" class="flex flex-col items-end">
              ...
              <span class="text-lg font-black text-emerald-600 leading-none">{{ p.reviewPivot.overall_score }}<span class="text-[10px] text-slate-300">/100</span></span>
            </div>
            <div v-else class="flex flex-col items-end">
              <span class="text-[11px] font-bold text-amber-500 animate-pulse  tracking-widest">Awaiting Score</span>
            </div>
          </div>
        </div>

        <div class="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between text-[10px] font-bold">
           <div class="flex items-center gap-3">
             <div class="flex flex-col">
               <span class="text-slate-400  text-[8px] tracking-widest mb-0.5">Assigned On</span>
               <span class="text-slate-700">{{ formatDate(p.reviewPivot?.assigned_at) }}</span>
             </div>
...
async function fetchProposals(page = 1) {
  ...
  try {
    ...
    error.value = err.response?.data?.message || 'Synchronization with core review service failed'

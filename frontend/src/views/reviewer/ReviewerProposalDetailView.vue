<template>
  <div class="space-y-6 animate-fade pb-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 py-2 border-b border-slate-100">
      <div>
        <router-link to="/app/reviewer/proposals" class="flex items-center gap-2 text-brand font-bold text-[10px] mb-3 hover:-translate-x-1 transition-transform">
        ...
        <p class="text-[10px] font-bold text-rose-500 mt-2  tracking-widest flex items-center gap-2">
          <span class="w-2 h-2 rounded-full bg-rose-500 shadow-lg shadow-rose-200 animate-pulse"></span>
          Double-Blind Protocol Active: Author identity is confidential
        </p>
      </div>
      <div v-if="!loading && proposal.id" class="flex items-center gap-3">
         <span class="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black  tracking-widest">
    ...
    <div v-else-if="error" class="card border-rose-100 bg-rose-50/30 p-12 text-center">
       ...
       <button @click="fetchProposal" class="btn bg-rose-600 text-white px-8 h-12 text-[10px]  font-black tracking-widest shadow-xl shadow-rose-200">Retry Authentication</button>
    </div>
    ...
    <template v-else>
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <!-- Core Data -->
          <div class="card p-8">
            <h2 class="text-[11px] font-black text-slate-800  tracking-widest mb-8 flex items-center gap-3">
            ...
            <div class="space-y-10">
              <div>
                <p class="text-[10px] font-bold text-slate-400  tracking-widest mb-3 ml-1">Research Nomenclature</p>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div>
                    <p class="text-[10px] font-bold text-slate-400  tracking-widest mb-3 ml-1">Thematic Focus</p>
                 ...
                 <div>
                    <p class="text-[10px] font-bold text-slate-400  tracking-widest mb-3 ml-1">Estimated Allocation</p>
              ...
              <div>
                <p class="text-[10px] font-bold text-slate-400  tracking-widest mb-3 ml-1">Expertise Tags</p>
                <div class="flex flex-wrap gap-2">
                   <span v-for="kw in keywordsList" :key="kw" class="px-4 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black text-slate-500  tracking-widest">
                     {{ kw }}
                   </span>
                </div>
              </div>
              ...
              <div>
                <p class="text-[10px] font-bold text-slate-400  tracking-widest mb-3 ml-1">Abstract Synopsis</p>
          ...
          <div class="card p-8">
            ...
            <div class="space-y-12">
               <div v-if="proposal.objectives">
                 <p class="text-[10px] font-bold text-slate-400  tracking-widest mb-4">Strategic Objectives</p>
               ...
               <div v-if="proposal.methodology">
                 <p class="text-[10px] font-bold text-slate-400  tracking-widest mb-4">Technical Methodology</p>
        ...
        <div class="space-y-6">
          ...
          <!-- Documentation -->
          <div class="card p-6">
             <h3 class="text-[11px] font-black text-slate-800  tracking-widest mb-5">Primary File</h3>
             <div v-if="proposal.file" class="p-4 bg-brand/5 border border-brand/10 rounded-2xl flex items-center justify-between group">
               <div class="flex items-center gap-3 min-w-0">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-[9px] font-bold text-brand  tracking-tighter">Verified Protocol</p>
                  </div>
               </div>
             ...
             <div v-else class="text-center py-4">
                <p class="text-[10px] font-bold text-rose-400 italic">No manuscript attached.</p>
             </div>
          </div>
          ...
          <div class="card overflow-hidden">
             <div class="px-6 py-4 bg-slate-900 border-b border-slate-800 flex items-center justify-between">
                <h3 class="text-[10px] font-black text-white  tracking-widest">Assessment Console</h3>
             ...
             <div v-if="alreadyReviewed" class="p-8 text-center bg-emerald-50/50">
                ...
                <p class="text-[10px] font-bold text-slate-400 mb-6 ">Assigned Score: {{ existingScore }}/5.0</p>
                <div class="p-3 bg-white border border-slate-100 rounded-2xl text-[9px] font-bold text-slate-500 italic">
             ...
             <div v-else class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-3">
                   <button @click="exportReviewTemplate" :disabled="downloading" class="flex flex-col items-center gap-2 p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:border-brand hover:bg-white transition-all group">
                      <svg class="w-5 h-5 text-slate-400 group-hover:text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V4" stroke-width="2"/></svg>
                      <span class="text-[8px] font-black  text-slate-500">{{ downloading ? 'Wait...' : 'Export Excel' }}</span>
                   </button>
                   <button @click="$refs.fileInput.click()" :disabled="uploading" class="flex flex-col items-center gap-2 p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:border-brand hover:bg-white transition-all group relative">
                      <input type="file" ref="fileInput" @change="handleFileUpload" accept=".xlsx,.xls" class="hidden" />
                      <svg class="w-5 h-5 text-slate-400 group-hover:text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-8l-4-4m0 0L8 8m4-4v12" stroke-width="2"/></svg>
                      <span class="text-[8px] font-black  text-slate-500">{{ uploading ? 'Wait...' : 'Import Excel' }}</span>
                   </button>
                </div>
                ...
                <form @submit.prevent="submitReview" class="space-y-5">
                   <div v-for="c in reviewCriteria" :key="c.id" class="space-y-2">
                      <div class="flex justify-between items-end">
                        <label class="text-[9px] font-black text-slate-500  tracking-widest">{{ c.name }}</label>
                        <span class="text-[11px] font-black text-brand">{{ scores[c.id] || 0 }}<span class="text-slate-300">/{{ c.max_score }}</span></span>
                      </div>
                   ...
                   <div class="space-y-2 pt-2">
                      <label class="text-[10px] font-black text-slate-800  tracking-widest">Protocol Decision</label>
                   ...
                   <div class="space-y-2 pt-2">
                       <label class="text-[10px] font-black text-slate-800  tracking-widest">Global Critique</label>
                       <textarea v-model="overallComments" required rows="4" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-xs font-medium text-slate-600 outline-none focus:ring-2 focus:ring-brand/20 resize-none" placeholder="Provide strategic feedback..."></textarea>
                   </div>

                   <button type="submit" :disabled="submitting" class="w-full py-4 bg-brand text-white rounded-2xl text-[10px] font-black  tracking-widest shadow-xl shadow-brand/20 hover:scale-[1.02] active:scale-95 transition-all">
                       {{ submitting ? 'Finalizing...' : 'Commit Review' }}
                   </button>
                </form>
             </div>
          </div>
...
async function fetchProposal() {
  ...
  try {
    ...
    error.value = err.response?.data?.message || 'Access Denied: Protocol is restricted'
...
async function exportReviewTemplate() {
  ...
  try {
    const response = await api.get(`/proposals/${route.params.id}/review-template`, { responseType: 'blob' })
    const url = window.URL.createObjectURL(new Blob([response.data]))
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', `RDRIMS-REVIEW-TEMPLATE-${proposal.value.id}.xlsx`)
    document.body.appendChild(link)
    link.click()
    link.remove()
    notif.success('Template generated')
  } catch (err) {
    notif.error('Export failed')
...
async function handleFileUpload(event) {
  ...
  try {
    ...
    if (confirm(`Excel Parsed. Score: ${data.overall_score}. Decision: ${data.decision_name}. Commit?`)) {
      await api.post(`/reviewer/proposals/${route.params.id}/review`, data)
      notif.success('Review synchronized')
      await fetchProposal()
    }
  } catch (err) {
    notif.error('Import failed')
...
async function submitReview() {
  ...
  try {
    ...
    notif.success('Review committed to ledger')
    await fetchProposal()
  } catch (err) {
    notif.error('Ledger commitment failed')

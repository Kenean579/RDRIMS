  ...
  <div class="flex flex-col gap-8 pb-12 animate-fade card">
    ...
    <form v-else @submit.prevent="handleSubmit" class="space-y-8 font-bold">
      ...
      <div class="card p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="md:col-span-2">
            <label class="block text-[11px] text-slate-500 uppercase tracking-widest mb-2 ml-1">Title *</label>
            <input v-model="form.title" type="text" required class="input h-12 font-black" />
          </div>
          <div>
            <label class="block text-[11px] text-slate-500 uppercase tracking-widest mb-2 ml-1">Proposal Type</label>
          ...
          <div>
            <label class="block text-[11px] text-slate-500 uppercase tracking-widest mb-2 ml-1">Budget (ETB) *</label>
      ...
      <div class="card p-8">
        ...
        <div class="space-y-6">
          <div>
            <label class="block text-[11px] text-slate-500 uppercase tracking-widest mb-2 ml-1">Keywords *</label>
            <input v-model="form.keywords" type="text" required class="input h-12 font-bold" placeholder="AI, Biology..." />
          </div>
          <div>
            <label class="block text-[11px] text-slate-500 uppercase tracking-widest mb-2 ml-1">Abstract *</label>
            <textarea v-model="form.abstract" required rows="4" class="input pt-3 font-medium resize-none"></textarea>
          </div>
          <div>
            <label class="block text-[11px] text-slate-500 uppercase tracking-widest mb-2 ml-1">Objectives *</label>
            <textarea v-model="form.objectives" required rows="4" class="input pt-3 font-medium resize-none"></textarea>
          </div>
          <div>
            <label class="block text-[11px] text-slate-500 uppercase tracking-widest mb-2 ml-1">Methodology *</label>
      ...
      <div class="card p-8">
        <div class="flex items-center justify-between mb-8">
          ...
          <button type="button" @click="addInvestigator" class="btn btn-secondary h-10 px-6 text-[10px] font-black uppercase tracking-widest border border-slate-200">
            Add Member
          </button>
        </div>
        ...
        <div v-if="form.investigators.length === 0" class="p-12 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200">
          <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">No co-investigators added.</p>
        </div>
        ...
        <div v-else class="space-y-4">
          <div v-for="(inv, index) in form.investigators" :key="index" class="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 relative group">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div class="md:col-span-2">
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">User Affiliation</label>
              ...
              <div v-if="!inv.user_id">
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Institution</label>

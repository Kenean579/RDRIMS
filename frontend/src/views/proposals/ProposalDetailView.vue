<template>
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div>
        <router-link to="/app/proposals" class="flex items-center gap-2 text-brand font-bold text-[10px] mb-3 hover:translate-x-1 transition-transform">
        ...
        <p class="text-slate-500 font-medium mt-1 text-[9px]">Submission details and tracking.</p>
      </div>
    ...
    <template v-else>
      ...
      <div class="card p-4 bg-slate-50/50 border border-slate-100 flex flex-wrap gap-4">
        ...
        <router-link v-if="(proposal.status?.name === 'draft' && (isOwner || auth.hasRole('super_admin'))) || auth.hasRole('super_admin','research_admin')" :to="`/app/proposals/${proposal.id}/edit`" class="btn bg-white border border-slate-100 text-slate-700 hover:text-brand hover:border-brand h-11 px-5 text-[11px] shadow-sm flex items-center gap-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
          Edit Details
        </router-link>

        <!-- Plagiarism Check: Admins Only -->
        <button v-if="auth.hasRole('super_admin','research_admin')" @click="checkOriginality" class="btn bg-indigo-600 hover:bg-indigo-700 text-white h-11 px-5 text-[11px]">Check Originality</button>
        
        <!-- Registration as Project: Approved Only + Admin -->
        <router-link v-if="auth.hasRole('super_admin','research_admin') && proposal.status?.name === 'approved'" :to="`/app/projects/create-from-proposal/${proposal.id}`" class="btn bg-teal-500 hover:bg-teal-600 text-white h-11 px-5 text-[11px] flex items-center justify-center">Convert To Project</router-link>
        
        <!-- Submit: Owner Only + Draft Only -->
        <button v-if="proposal.status?.name === 'draft' && isOwner" @click="submitProposal" class="btn btn-primary h-11 px-5 text-[11px]">Submit Final</button>
        ...
        <template v-if="auth.hasRole('super_admin','research_admin') && ['submitted','under_review','finance_check','ethics_pending'].includes(proposal.status?.name)">
          <button @click="showAssignReviewers = true" class="btn bg-amber-500 hover:bg-amber-600 text-white h-11 px-5 text-[11px]">Assign Reviewers</button>
          <button @click="exportScoresMatrix" class="btn bg-indigo-600 hover:bg-indigo-700 text-white h-11 px-5 text-[11px]">Export Scores Matrix</button>
          <button v-if="proposal.status?.name === 'submitted'" @click="sendToFinance" class="btn bg-slate-700 hover:bg-slate-800 text-white h-11 px-5 text-[11px]">Send to Finance</button>
          <button v-if="proposal.status?.name === 'finance_check'" @click="generateEthics" class="btn bg-slate-700 hover:bg-slate-800 text-white h-11 px-5 text-[11px]">Generate Ethics IRB</button>
          
          <button v-if="canApprove" @click="approveProposal" class="btn bg-emerald-500 hover:bg-emerald-600 text-white h-11 px-5 text-[11px]">Final Approve</button>
          <button v-if="canApprove" @click="showReject = true" class="btn bg-rose-600 hover:bg-rose-700 text-white h-11 px-5 text-[11px]">Reject</button>
        </template>

        <!-- Finance Officer Check -->
        <template v-if="auth.hasRole('finance_officer') && proposal.status?.name === 'finance_check'">
          <button @click="approveProposal" class="btn bg-emerald-500 hover:bg-emerald-600 text-white h-11 px-5 text-[11px]">Verify Budget</button>
          <button @click="showReject = true" class="btn bg-rose-600 hover:bg-rose-700 text-white h-11 px-5 text-[11px]">Budget Revision</button>
        </template>

        <!-- Ethics Officer Check -->
        <template v-if="auth.hasRole('ethics_officer') && proposal.status?.name === 'ethics_pending'">
          <button @click="approveProposal" class="btn bg-emerald-500 hover:bg-emerald-600 text-white h-11 px-5 text-[11px]">Ethical Clearance</button>
          <button @click="showReject = true" class="btn bg-rose-600 hover:bg-rose-700 text-white h-11 px-5 text-[11px]">Ethics Rejection</button>
        </template>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 font-bold">
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div class="card p-8/20">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
              <div>
                <dt class="text-[10px] font-medium text-slate-400 mb-1.5 ml-1">Research Title</dt>
                <dd class="text-sm font-bold text-slate-800 bg-slate-50 p-4 rounded-2xl border border-slate-100">{{ proposal.title }}</dd>
              </div>
               <div>
                <dt class="text-[10px] font-medium text-slate-400 mb-1.5 ml-1">Year & Allocation</dt>
              ...
              <div>
                <dt class="text-[10px] font-medium text-slate-400 mb-1.5 ml-1">Work Type</dt>
                <dd class="p-4 rounded-2xl bg-white border border-slate-100">
                  <span class="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-2xl text-[10px] font-medium">{{ proposal.type?.name || 'N/A' }}</span>
                </dd>
              </div>
              <div>
                <dt class="text-[10px] font-medium text-slate-400 mb-1.5 ml-1">Principal Investigator</dt>
                <dd class="p-4 rounded-2xl bg-white border border-slate-100 text-slate-700 flex items-center gap-2">
                  ...
                  <span v-if="proposal.submitted_by?.department" class="text-[10px] font-medium text-slate-400">({{ proposal.submitted_by.department.name }})</span>
                </dd>
              </div>
            </dl>
          </div>
          ...
          <div class="card p-8">
            ...
            <div class="space-y-10">
              <div>
                <p class="text-[10px] font-medium text-slate-400 mb-3 ml-1">Domain Keywords</p>
                <div class="flex flex-wrap gap-2">
                  <span v-for="kw in proposal.keywords?.split(',')" :key="kw" class="px-3 py-1.5 bg-slate-50 text-slate-600 border border-slate-100 rounded-2xl text-[10px] font-medium hover:bg-white hover:border-brand transition-colors cursor-default">{{ kw.trim() }}</span>
                </div>
              </div>
              <div>
              ...
              <div v-if="proposal.objectives">
                <p class="text-[10px] font-medium text-slate-400 mb-3 ml-1">Specific Objectives</p>
               ...
               <div v-if="proposal.methodology">
                <p class="text-[10px] font-medium text-slate-400 mb-3 ml-1">Methodology</p>
        ...
        <div class="flex flex-col gap-5">
          ...
          <div class="card p-8">
            ...
            <div v-if="proposal.investigators?.length" class="space-y-4">
              <div v-for="inv in proposal.investigators" :key="inv.id" class="flex items-start gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-white hover:border-brand transition-all group shadow-sm">
                ...
                <div class="min-w-0">
                  ...
                  <p class="text-[9px] font-medium text-slate-400 mt-0.5">{{ inv.role?.name || 'Researcher' }}</p>
                  
                  <div class="mt-3 space-y-1">
                    <p v-if="inv.email || inv.user?.email" class="text-[10px] text-slate-500 font-medium flex items-center gap-1.5">
                    ...
                    <p v-if="inv.institution" class="text-[10px] text-slate-500 font-medium flex items-center gap-1.5 italic">
            ...
            <p v-else class="text-[10px] font-medium text-slate-400 italic text-center py-6">No co-investigators.</p>
          </div>
          ...
          <div class="card p-8">
            ...
            <div v-if="proposal.file" class="p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex items-center justify-between group">
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                   ...
                   <p class="text-[9px] font-medium text-emerald-600 mt-0.5">Formal Proposal Attached</p>
                </div>
              </div>
            ...
            <div v-if="proposal.status?.name === 'draft' || !proposal.file || auth.hasRole('super_admin')" class="mt-4">
              ...
              <button v-if="selectedFile" @click="uploadDocument" :disabled="uploading" class="btn btn-primary w-full mt-4 h-10 text-[10px] tracking-widest ">
          ...
          <div class="card p-8">
            ...
            <div class="space-y-6">
              ...
              <div v-if="proposal.reviewers?.length" class="space-y-3">
                 <p class="text-[10px] font-medium text-slate-400">Internal Peer Reviewers</p>
                 <div v-for="r in proposal.reviewers" :key="r.id" class="p-4 bg-slate-50 rounded-2xl border border-slate-100 relative group">
                    ...
                    <p class="text-[9px] font-medium  text-slate-400 tracking-widest">{{ r.pivot?.overall_score ? 'Feedback Provided' : 'Under Review' }}</p>
                 </div>
              </div>
              <p v-else class="text-[10px] font-medium text-slate-400  italic">No reviewers assigned.</p>

              <hr class="border-slate-50" />

              <!-- External Checks -->
              <div class="space-y-4">
                 <p class="text-[10px] font-medium text-slate-400">Procedural Status</p>
                 <div class="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-100">
                    <span class="text-[10px] font-medium  text-slate-600">Ethics Clearance</span>
                    <StatusBadge :status="proposal.ethics_status || 'not_requested'" size="sm" />
                 </div>
                 <div class="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-100">
                    <span class="text-[10px] font-medium  text-slate-600">Finance Audit</span>
    ...
    <Modal :show="showAssignReviewers" title="Assign Peer Reviewers" @close="showAssignReviewers = false">
       <div class="space-y-6">
          <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100">
             <p class="text-[10px] font-medium text-slate-400 mb-4">Eligible Reviewers (Expertise Match)</p>
                ...
                <label v-for="r in availableReviewers" :key="r.user ? r.user.id : r.id" class="flex items-center justify-between p-3 bg-white rounded-2xl border border-slate-100 hover:border-brand cursor-pointer group transition-all">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                      ...
                      <p class="text-[9px] font-medium text-slate-400 ">{{ r.user ? r.user.email : r.email }}</p>
                    </div>
                  </div>
                  <div v-if="r.match_percentage" class="px-2 py-1 bg-brand/10 text-brand text-[10px] font-medium rounded flex flex-col items-center justify-center">
...
onMounted(async () => {
  await fetchProposal()
  try {
    if (auth.hasPermission('assign_reviewers') || auth.hasRole('super_admin', 'research_admin')) {

    ...
    📄 Download Proposal 
          ...
          <div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
             ...
             <div class="flex gap-2 items-center">
               <router-link
                 v-if="p.status?.name === 'approved' && !p.project?.id && (auth.hasRole('super_admin','research_admin','campus_admin','faculty_admin','director','department_head'))"
                 :to="`/app/projects/create-from-proposal/${p.id}`"
                 @click.stop
                 class="px-3 py-1.5 bg-teal-500 hover:bg-teal-600 text-white rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1"
               >
                 Convert To Project
               </router-link>
               <router-link
                 v-else-if="p.project?.id"
                 :to="`/app/projects/${p.project.id}`"
                 @click.stop
                 class="px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl text-xs font-bold flex items-center gap-1 hover:bg-emerald-100 transition-colors"
               >
                 ✓ View Project
               </router-link>

               <ActionMenu :actions="[
                 { key: 'view', label: 'View Proposal', handler: () => $router.push(`/app/proposals/${p.id}`) },
                 { key: 'convert', label: 'Convert To Project', show: p.status?.name === 'approved' && !p.project?.id && (auth.hasRole('super_admin','research_admin','campus_admin','faculty_admin','director','department_head')), handler: () => $router.push(`/app/projects/create-from-proposal/${p.id}`) },
                 { key: 'view_project', label: 'View Associated Project', show: !!p.project?.id, handler: () => $router.push(`/app/projects/${p.project.id}`) },

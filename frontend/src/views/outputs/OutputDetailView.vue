<template>
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div>
        <router-link to="/app/outputs" class="flex items-center gap-2 text-indigo-600 font-bold capitalize tracking-widest text-[10px] mb-3 hover:translate-x-1 transition-transform">
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div class="card p-5 space-y-8">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
              <div>
                <dt class="text-[10px] font-bold text-slate-400 capitalize tracking-widest mb-1.5 ml-1">Title & Identification</dt>
                <dd class="text-sm font-bold text-slate-800 bg-slate-50 p-4 rounded-xl border border-slate-100">{{ output.title }}</dd>
              </div>
              <div>
                <dt class="text-[10px] font-bold text-slate-400 capitalize tracking-widest mb-1.5 ml-1">Classification</dt>
                <dd class="p-4 rounded-xl bg-white border border-slate-100 flex flex-wrap gap-2">
                  <span class="px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-bold capitalize tracking-widest">{{ output.category?.name }}</span>
                  <span class="px-2.5 py-1 bg-slate-50 text-slate-500 rounded-lg text-[10px] font-bold capitalize tracking-widest">{{ output.subtype?.name }}</span>
                </dd>
              </div>
              <div v-if="output.student_level">
                <dt class="text-[10px] font-bold text-slate-400 capitalize tracking-widest mb-1.5 ml-1">Academic Level</dt>
              ...
              <div v-if="output.project">
                <dt class="text-[10px] font-bold text-slate-400 capitalize tracking-widest mb-1.5 ml-1">Associated Project</dt>
            ...
            <div v-if="output.feedback" class="mt-4 bg-amber-50/50 border border-amber-100 rounded-2xl p-6">
              <p class="text-[10px] font-bold text-amber-600 capitalize tracking-widest mb-2">Reviewer Feedback</p>
              <p class="text-sm font-semibold text-amber-900 italic">{{ output.feedback }}</p>
            </div>
          </div>

          <!-- Institutional Approval Workflow -->
          <div class="card p-5 bg-white border border-slate-100 shadow-sm relative overflow-hidden">
            <div class="absolute top-0 right-0 w-32 h-32 bg-brand/5 rounded-full -translate-y-1/2 translate-x-1/2"></div>
            <h2 class="text-xs font-bold text-slate-400 capitalize tracking-widest mb-6 flex items-center gap-2 relative z-10">
              <span class="w-1 h-3 bg-brand rounded-full"></span>
              Institutional Approval Workflow
            </h2>
            
            <div class="flex items-center gap-4 mb-5">
              <div class="p-3 bg-slate-50 rounded-2xl border border-slate-100 italic text-[10px] font-bold text-slate-400 capitalize tracking-widest">Current State</div>
              <StatusBadge :status="output.status?.name || 'draft'" size="lg" />
            </div>
            
            <div class="flex flex-wrap gap-4 font-bold capitalize tracking-widest">
               <!-- Student Submission -->
               <button v-if="output.status?.name === 'draft'" @click="changeStatus(2)" class="btn bg-brand hover:bg-indigo-700 text-white h-11 px-5 text-[11px] shadow-lg shadow-brand/20">Submit Final Output</button>
               
               <!-- Supervisor Approval -->
               <template v-if="output.status?.name === 'submitted'">
                 <button @click="changeStatus(3)" class="btn bg-emerald-500 hover:bg-emerald-600 text-white h-11 px-5 text-[11px] shadow-lg shadow-emerald-500/20">Supervisor Clearance</button>
                 <button @click="changeStatus(5)" class="btn bg-rose-500 hover:bg-rose-600 text-white h-11 px-5 text-[11px] shadow-lg shadow-rose-500/20">Reject Submission</button>
               </template>

               <!-- Department Head Final Sign-off -->
               <template v-if="output.status?.name === 'approved_by_supervisor'">
                 <button @click="changeStatus(4)" class="btn bg-indigo-600 hover:bg-indigo-700 text-white h-11 px-5 text-[11px] shadow-lg shadow-indigo-600/20">Final Dept Approval</button>
                 <button @click="changeStatus(5)" class="btn bg-rose-500 hover:bg-rose-600 text-white h-11 px-5 text-[11px] shadow-lg shadow-rose-500/20">Reject Output</button>
               </template>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-5">
          ...
          <div class="card p-5 bg-white border border-slate-100 shadow-sm">
            <div class="flex items-center justify-between mb-6">
              ...
              <button @click="showAddParticipant = true" class="text-[10px] font-bold text-brand capitalize tracking-widest hover:underline">+ Assign</button>
            </div>
            ...
            <div v-if="output.participants?.length" class="space-y-4">
              <div v-for="p in output.participants" :key="p.id" class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 group hover:border-brand/30 transition-all">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 bg-brand-light text-brand rounded-lg flex items-center justify-center text-[10px] font-bold capitalize shadow-inner">{{ p.user?.name?.substring(0,1) }}</div>
                  <div class="min-w-0">
                    <p class="text-sm font-bold text-slate-800 leading-tight truncate">{{ p.user?.name }}</p>
                    <p class="text-[9px] font-bold text-slate-400 capitalize tracking-widest mt-0.5">{{ p.participant_type?.name }}</p>
                  </div>
                </div>
            ...
            <p v-else class="text-[10px] font-bold text-slate-400 capitalize tracking-widest italic text-center py-6">No contributors assigned.</p>
          </div>
        </div>
      </div>
    </template>

  ...
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <div class="card p-8 flex flex-col md:flex-row gap-5 items-end">
      <div class="flex-1 w-full relative">
        <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Search Files</label>
       ...
       <div class="w-full md:w-64">
        <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Mime Group</label>
    ...
    <div v-else class="card overflow-hidden">
      <div class="overflow-x-auto">
        <table class="table-auto">
          <thead>
            <tr>
              <th class="pl-8 py-4 text-[10px] font-medium text-slate-400">Filename</th>
              <th class="py-4 text-[10px] font-medium text-slate-400">Size</th>
              <th class="py-4 text-[10px] font-medium text-slate-400">Mime Type</th>
              <th class="py-4 text-[10px] font-medium text-slate-400">Stored On</th>
              <th class="pr-8 py-4 text-[10px] font-medium text-slate-400 text-right">Actions</th>
            </tr>
          </thead>
              ...
              <td class="pl-8 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-[9px] font-medium text-slate-400">{{ file.path }}</p>
                  </div>
                </div>
              </td>
              <td class="py-4 text-xs font-bold text-slate-600">{{ formatSize(file.size) }}</td>
              <td class="py-4 font-bold">
                <span class="px-2 py-0.5 bg-slate-100 text-slate-500 rounded text-[9px] border border-slate-100">{{ file.mime_type }}</span>
              </td>
              <td class="py-4 text-xs font-medium text-slate-400">{{ formatDate(file.created_at) }}</td>
              <td class="pr-8 py-4 text-right">
                 <button @click="downloadFile(file)" class="btn btn-ghost text-brand text-[10px] font-medium py-1.5 px-4 h-auto border border-brand hover:bg-brand hover:text-white transition-all shadow-sm">
                   Download
                 </button>
              </td>
            </tr>

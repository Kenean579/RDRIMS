    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <div v-for="faculty in faculties" :key="faculty.id" class="card p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 card-hover transition-all">
        ...
        <div class="flex gap-2 shrink-0">
          <button @click="editFaculty(faculty)" class="btn btn-secondary h-9 px-4 text-xs font-medium">
            Edit
          </button>
          <button @click="confirmDelete(faculty)" class="btn btn-danger h-9 px-4 text-xs font-medium">
            Delete
          </button>
        </div>
      </div>

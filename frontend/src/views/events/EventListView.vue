      ...
      <div v-for="event in events" :key="event.id" class="card overflow-hidden flex flex-col group card-hover border-b-4 border-b-brand/10 hover:border-b-brand transition-all">
        <div class="p-8 flex-1">
          <div class="flex justify-between items-start mb-6">
            <span class="px-3 py-1 bg-brand-light text-brand text-[9px] font-black uppercase tracking-widest rounded-lg border border-brand/20 shadow-sm">{{ event.type?.name || 'Workshop' }}</span>
          </div>
        ...
        <div class="p-5 bg-slate-50/50 border-t border-slate-100 flex gap-3">
          <button @click="viewEvent(event)" class="flex-1 btn btn-secondary justify-center text-[10px] font-black uppercase tracking-widest py-3">Open</button>
          <button v-if="!event.is_registered" @click="registerForEvent(event)" class="flex-1 btn btn-primary justify-center text-[10px] font-black uppercase tracking-widest py-3 shadow-lg shadow-blue-500/20">Join Now</button>
          <span v-else class="flex-1 text-center py-3 text-[10px] font-black uppercase tracking-widest text-emerald-600 bg-emerald-50 rounded-xl border border-emerald-100 shadow-sm">Going ✓</span>
        </div>
      </div>
    </div>
      ...
      <div v-if="selectedEvent" class="flex flex-col gap-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-8 bg-slate-900 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden group">
          ...
          <div class="relative z-10">
            <p class="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Begins</p>
            <p class="text-sm font-black text-white tracking-tight">{{ formatDate(selectedEvent.start_date) }}</p>
          </div>
          <div class="relative z-10">
            <p class="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Ends</p>
            <p class="text-sm font-black text-white tracking-tight">{{ formatDate(selectedEvent.end_date) }}</p>
          </div>
          <div class="relative z-10">
            <p class="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Where</p>
            <p class="text-sm font-black text-white tracking-tight">{{ selectedEvent.location || 'Online / Remote' }}</p>
          </div>
          <div class="relative z-10">
            <p class="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Max Capacity</p>
        ...
        <div v-if="selectedEvent.organizer" class="pt-8 border-t border-slate-100 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Posted by</p>
              <p class="text-sm font-black text-slate-900 tracking-tight">{{ selectedEvent.organizer.name }}</p>
            </div>
          </div>
          <button @click="selectedEvent = null" class="btn btn-secondary px-8 h-11 text-[11px] font-black uppercase tracking-widest">Close</button>
        </div>
      </div>
    </Modal>
  </div>

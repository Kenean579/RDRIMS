<template>
  <div class="flex flex-col gap-5 pb-6 animate-fade card">
    ...
    <Modal :show="!!selectedEvent" :title="selectedEvent?.title" size="lg" @close="selectedEvent = null">
      <div v-if="selectedEvent" class="flex flex-col gap-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-slate-900 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden group">
          <div class="absolute inset-0 bg-brand/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
          <div class="relative z-10">
            <p class="text-[9px] font-bold text-slate-500 capitalize tracking-widest mb-2">Begins</p>
            <p class="text-sm font-bold text-white tracking-tight">{{ formatDate(selectedEvent.start_date) }}</p>
          </div>
          <div class="relative z-10">
            <p class="text-[9px] font-bold text-slate-500 capitalize tracking-widest mb-2">Ends</p>
            <p class="text-sm font-bold text-white tracking-tight">{{ formatDate(selectedEvent.end_date) }}</p>
          </div>
          <div class="relative z-10">
            <p class="text-[9px] font-bold text-slate-500 capitalize tracking-widest mb-2">Where</p>
            <p class="text-sm font-bold text-white tracking-tight">{{ selectedEvent.location || 'Online / Remote' }}</p>
          </div>
          <div class="relative z-10">
            <p class="text-[9px] font-bold text-slate-500 capitalize tracking-widest mb-2">Max Capacity</p>
            <p class="text-sm font-bold text-white tracking-tight">{{ selectedEvent.max_participants || 'Unlimited' }}</p>
          </div>
        </div>
        <div v-if="selectedEvent.organizer" class="pt-4 border-t border-slate-100 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 font-bold">@</div>
            <div>
              <p class="text-[10px] font-bold text-slate-400 capitalize tracking-widest">Posted by</p>
              <p class="text-sm font-bold text-slate-900 tracking-tight">{{ selectedEvent.organizer.name }}</p>
            </div>
          </div>
          <button @click="selectedEvent = null" class="btn btn-secondary px-5 h-11 text-[11px] font-bold capitalize tracking-widest">Close</button>
        </div>
      </div>
    </Modal>
  </div>

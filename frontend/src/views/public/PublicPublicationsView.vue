  ...
  <div class="flex flex-col gap-5 pb-6 animate-fade">
    ...
    <div class="card p-5 flex flex-col md:flex-row gap-5 items-end">
      <div class="flex-1 w-full relative">
        <label class="block text-[11px] text-slate-500 font-bold capitalize tracking-widest mb-2 ml-1">Search Publications</label>
    ...
    <div v-else-if="filteredPubs.length === 0" class="card p-6 text-center text-slate-400 text-xs font-bold capitalize tracking-widest italic">
      No publications found in our repository.
    </div>

    <div v-else class="space-y-4">
      <div v-for="pub in filteredPubs" :key="pub.id" class="card p-4 flex flex-col md:flex-row gap-4 group card-hover relative border-l-4 border-l-transparent hover:border-l-brand transition-all">
        <div class="w-16 h-20 rounded-xl overflow-hidden shrink-0 bg-blue-50 border border-slate-100 shadow-sm flex items-center justify-center">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex flex-wrap items-center gap-4 text-[10px] font-bold capitalize tracking-widest text-slate-400">
        ...
        <div class="flex items-end">
          <a v-if="pub.doi || pub.scholar_url" :href="pub.scholar_url || `https://doi.org/${pub.doi}`" target="_blank" class="btn btn-secondary text-[10px] font-bold capitalize tracking-widest px-4 h-9">
            Read Online
          </a>
        </div>
      </div>

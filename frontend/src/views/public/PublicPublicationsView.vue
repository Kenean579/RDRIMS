  ...
  <div class="flex flex-col gap-5 pb-6 animate-fade">
    ...
    <div class="card p-8 flex flex-col md:flex-row gap-5 items-end">
      <div class="flex-1 w-full relative">
        <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Search Publications</label>
    ...
    <div v-else-if="filteredPubs.length === 0" class="card p-8 text-center text-slate-400 text-xs italic">
      No publications found in our repository.
    </div>

    <div v-else class="space-y-4">
      <div v-for="pub in filteredPubs" :key="pub.id" class="card p-4 flex flex-col md:flex-row gap-4 group card-hover relative border-l-4 border-l-transparent transition-all">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-sm font-medium text-slate-500 mb-4">{{ pub.authors?.map(a => a.user?.name || a.name).join(', ') }}</p>
          
          <div class="flex flex-wrap items-center gap-4 text-[10px] font-medium text-slate-400">
        ...
        <div class="flex items-end">
          <a v-if="pub.doi || pub.scholar_url" :href="pub.scholar_url || `https://doi.org/${pub.doi}`" target="_blank" class="btn btn-secondary text-[10px] font-medium px-4 h-9">
            Read Online
          </a>
        </div>
      </div>

<template>
  <div class="flex flex-col gap-8 pb-12 animate-fade">
    ...
    <div v-else-if="events.length === 0" class="card p-12 text-center text-slate-400 text-xs font-black capitalize tracking-widest italic">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div v-for="event in events" :key="event.id" class="card overflow-hidden group card-hover border-l-4 border-l-transparent hover:border-l-brand transition-all flex flex-col">
        <div class="bg-slate-900 p-6 text-white flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div class="text-center bg-white/10 rounded-xl p-2 min-w-[60px]">
              ...
              <p class="text-[9px] font-black capitalize tracking-widest opacity-60">{{ getMonth(event.start_date) }}</p>
            </div>
            <div>
          ...
          <div class="hidden sm:block">
            <span class="px-2 py-1 bg-brand text-[9px] font-black capitalize tracking-widest rounded-lg">Featured</span>
          </div>
        </div>
        ...
        <div class="p-6 flex-1 flex flex-col">
          ...
          <div class="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between text-[10px] font-black capitalize tracking-widest text-slate-400">

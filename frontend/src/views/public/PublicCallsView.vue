  ...
  <div class="flex flex-col gap-5 pb-6 animate-fade">
    ...
    <div class="card p-8 flex flex-col md:flex-row gap-5 items-end">
      <div class="flex-1 w-full relative">
        <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Search Database</label>
      ...
      <div class="w-full md:w-64">
        <label class="block text-[11px] text-slate-500 font-medium mb-2 ml-1">Call Status</label>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      >
        <div class="flex items-center gap-2 mb-4">
          <span class="px-2 py-0.5 bg-brand-light text-brand text-[9px] font-medium rounded border border-brand/10">{{ call.status?.name || 'Open' }}</span>
          <span v-if="isUrgent(call.deadline)" class="px-2 py-0.5 bg-rose-50 text-rose-600 text-[9px] font-medium rounded border border-rose-100">Expiring Soon</span>
        </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-50 text-[10px] font-medium text-slate-400">

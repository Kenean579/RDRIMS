<template>
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-5 py-6 animate-fade">
    ...
    <!-- Error State -->
    <div v-else-if="!call" class="card p-6 text-center shadow-sm">
      <div class="w-16 h-16 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-6 text-2xl">⚠️</div>
      <h1 class="text-2xl font-bold text-slate-800 mb-2">Call Not Found</h1>
      <p class="text-slate-500 mb-5 max-w-sm mx-auto">This research call may have been removed or the link is invalid.</p>
      <router-link to="/calls" class="btn btn-secondary px-5">Back to Calls</router-link>
    </div>
    ...
    <template v-else>
      ...
      <!-- Main Header Card -->
      <div class="card p-5 md:p-5 mb-5 relative border-t-4 border-t-brand shadow-sm">
        <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div>
            <div class="flex items-center gap-3 mb-4">
              <span class="px-3 py-1 bg-brand/10 text-brand text-xs font-bold capitalize tracking-widest rounded border border-brand/20">
                {{ call.status?.name || 'Open' }}
              </span>
            </div>
            <h1 class="text-xl md:text-2xl font-bold text-slate-900 tracking-tight leading-tight mb-4">
              {{ call.title }}
            </h1>
          ...
          <div class="shrink-0 md:text-right">
            <template v-if="call.status?.name === 'open' || !call.status">
              <router-link :to="`/app/proposals/create?call_id=${call.id}`" class="btn btn-primary px-5 py-3.5 shadow-lg shadow-brand/20 w-full md:w-auto text-base">
                Apply Now
              </router-link>
              <p class="text-xs text-slate-400 font-medium mt-3 text-center md:text-right">Login required to apply</p>
            </template>
            <div v-else class="px-6 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-500 font-bold text-sm capitalize">
              Submissions Closed
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="space-y-6">
          <div class="card p-6 shadow-sm bg-slate-50 border border-slate-100">
            <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Overview</h3>
            <ul class="space-y-4">
              <li>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1.5">Deadline</p>
              ...
              <li class="pt-4 border-t border-slate-200">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1.5">Budget Limit</p>
                <p class="text-lg font-bold text-emerald-600 tracking-tight">
                  {{ formatCurrency(call.budget_limit) }}
                </p>
              </li>
              <li class="pt-4 border-t border-slate-200">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1.5">Academic Cycle</p>
          ...
          <div class="card p-6 shadow-sm border border-slate-100 relative overflow-hidden group">
            ...
            <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 relative z-10">Need Assistance?</h3>

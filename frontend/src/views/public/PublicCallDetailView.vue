<template>
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-5 py-6 animate-fade">
    ...
    <!-- Error State -->
    <div v-else-if="!call" class="card p-8 text-center shadow-sm">
    ...
    <!-- Content -->
    <template v-else>
      <router-link to="/calls" class="inline-flex items-center text-sm font-medium text-slate-500 hover:text-brand mb-6 transition-colors">
        ← Back to Call for Proposals
      </router-link>

      <!-- Main Header Card -->
      <div class="card p-8 md:p-5 mb-5 relative shadow-sm">
        <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div>
            <div class="flex items-center gap-3 mb-4">
              <span class="px-3 py-1 bg-brand/10 text-brand text-xs font-bold rounded border border-brand/20">
          ...
          <div class="shrink-0 md:text-right">
            <template v-if="call.status?.name === 'open' || !call.status">
              <router-link :to="`/app/proposals/create?call_id=${call.id}`" class="btn btn-primary px-5 py-3.5 w-full md:w-auto text-base">
                Apply Now
              </router-link>
              <p class="text-xs text-slate-400 font-medium mt-3 text-center md:text-right">Login required to apply</p>
            </template>
            <div v-else class="px-6 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-slate-500 font-bold text-sm ">
              Submissions Closed
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <!-- Sidebar Info -->
        <div class="space-y-6">
          <div class="card p-8 shadow-sm bg-slate-50 border border-slate-100">
          ...
          <div class="card p-8 shadow-sm border border-slate-100 relative overflow-hidden group">
            <div class="absolute inset-0 bg-brand/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
            <h3 class="text-xs font-medium text-slate-400 mb-2 relative z-10">Need Assistance?</h3>

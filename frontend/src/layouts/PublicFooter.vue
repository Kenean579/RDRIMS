<template>
  <footer class="bg-slate-900 pt-4 pb-4 border-t border-slate-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-5">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div class="md:col-span-1">
          ...
          <p class="text-sm font-semibold text-slate-400 leading-relaxed">
            {{ appDescription }}
          </p>
        </div>
        ...
        <div>
          ...
          <ul class="space-y-4">
            <li v-if="contactEmail" class="flex items-start gap-3 text-slate-400">
              ...
              <span class="text-sm font-semibold break-all">{{ contactEmail }}</span>
            </li>
            <li v-if="contactPhone" class="flex items-start gap-3 text-slate-400">
              <svg class="w-5 h-5 shrink-0 mt-0.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
              </svg>
              <span class="text-sm font-semibold">{{ contactPhone }}</span>
            </li>
            <li v-if="contactAddress" class="flex items-start gap-3 text-slate-400">
              <svg class="w-5 h-5 shrink-0 mt-0.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
              </svg>
              <span class="text-sm font-semibold">{{ contactAddress }}</span>
            </li>
          </ul>
        </div>
      </div>

...
if (!$pivot) {
    ...
    public function downloadTemplate(Proposal $proposal, Request $request): StreamedResponse
    {
        ...
        $sheet->setCellValue('B' . $row, 'Decision ID (See below)');
        $sheet->setCellValue('C' . $row, 'Overall Comments');
        
        $decisions = ReviewDecision::all();
        $row++;
        $decisionRowStart = $row;
